...
def check_local_vs_redis() -> Dict:
    ...
    if result["local_hash"] and result["redis_hash"]:
        ...
        if result["local_hash"] == result["redis_hash"]:
            ...
            # Values match but timestamps differ - one side is stale, not split-brain
            result["status"] = "stale"
            result["warning"] = f"State stale (ts-drift={ts_drift:.0f}s but values match)"
        elif p_drift > 0.05:
...
def check_heartbeat_sync() -> Dict:
    ...
    if result["local_age"] is not None:
        if result["local_age"] > HEARTBEAT_STALE_THRESHOLD:
            result["status"] = "STALE"
