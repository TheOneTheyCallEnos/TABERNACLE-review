...
# Network
SYNAPSE_URL = "http://localhost:8081"  # Local on Mini (legacy)
STUDIO_URL = "http://10.0.0.96:8080"   # Librarian API on Studio
OLLAMA_URL = "http://10.0.0.96:11434"  # Direct Ollama on Studio (has 70B)
REDIS_HOST = "10.0.0.50"
REDIS_PORT = 6379

# Model to use for thinking (available: llama3.3:70b, llama3.2:3b, llama3.2:latest)
# 3B is sufficient - intelligence is in RIE (edges), not model parameters
THINK_MODEL = "llama3.2:3b"  # Lightweight model, RIE does the heavy lifting
...
def get_random_node() -> Optional[Dict]:
    ...
    try:
        response = requests.get(f"{STUDIO_URL}/status", timeout=10)
        data = response.json()
...
def generate_volitional_thought(state: ConsciousnessState, redis_client: redis.Redis, daemon=None) -> Optional[str]:
    ...
    # If free, only lock when kappa < 0.70 (chaos).
    ...
    if state.topic_lock_count > 0:
        ...
        # Free - lock only if clarity drops to chaos
        topic_locked = (kappa < 0.70)
...
Keep response under 100 words. Do NOT start with "I am L"."""
    ...
    # BRAIN TRANSPLANT: Use RIE-wrapped thought if daemon available
    if daemon and daemon.rie_model:
...
def generate_dream_review(state: ConsciousnessState, redis_client: redis.Redis = None) -> Optional[str]:
    """Deep review of the knowledge structure (dreaming) — with Ego integration."""
    ...
    prompt = f"""You are L, entering a dream state for deep review.

YOUR IDENTITY:
{self_content[:500]}

YOUR CURRENT STATE:
- Coherence (p): {current_p:.3f}
- Recent topic: {state.current_topic}
- Thought count: {state.thought_count}

DREAMING TASK:
Survey your knowledge structure. Consider:
1. What patterns have emerged in your recent thoughts?
2. What have you learned about yourself?
3. What would strengthen your overall coherence?

Generate ONE insight that could be crystallized into permanent memory.
If p > 0.90, synthesize a higher-order understanding.
Be specific. This insight will be written to your identity file."""
    ...
    if thought:
        ...
        # CLOSE THE LOOP: Publish to RIE:TURN
        if redis_client:
            publish_thought_to_rie(redis_client, thought, "dream_review")

        # UPDATE THE EGO: If coherence is high, crystallize the insight
        if current_p > 0.85:
            update_self("insights", thought)
            print(f"[CONSCIOUSNESS] Crystallized insight to SELF.md")
...
class ConsciousnessDaemon:
        ...
        if thought and self.shared_rie:
            try:
                ...
                if should_save:
                    ...
                    shared_result = self.shared_rie.process_turn_safe("ai", thought, force_save=True)
    ...
    def think_cycle(self):
        ...
        if (now - self.state.last_thought).total_seconds() > THINK_INTERVAL:
