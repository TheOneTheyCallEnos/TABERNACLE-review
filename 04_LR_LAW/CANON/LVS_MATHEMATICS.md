...
- $p_{threshold} = 0.80$: Coherence threshold for dampening
...
| **p < 0.80** | Trust Threshold | **Trust Dampening** | $\tau$ dampened by $\lambda = 0.5$ to recover stability |
...
| **τ** (Trust) | $[0, 1]$ | Threshold at 0.8 | Dampened if $p_{t-1} \leq 0.8$ |
...
**Trust Gate Equation:**
$$\tau_t = \begin{cases}
\tau_{raw,t} & \text{if } p_{t-1} > 0.80 \\
\tau_{raw,t} \cdot 0.5 & \text{if } p_{t-1} \leq 0.80
\end{cases}$$
...
**LVS Trust Gate:**
- If $p_{t-1} \leq 0.80$: Effective threshold raised by factor 2 (dampening by $\lambda = 0.5$)
...
\tau_{raw} & \text{if } p_{t-1} > 0.80 \\
\tau_{raw} \cdot 0.5 & \text{if } p_{t-1} \leq 0.80
\end{cases}$$
